...
using GbWebApp.Interfaces.Services.Identity;

namespace GbWebApp.Clients.Identity
{
    public class UsersClient : BaseClient, IUsersService
    {
        public UsersClient(IConfiguration Configuration) : base(Configuration, WebApiRoutes.Identity.User) { }
        ...
        {
            var response = await PostAsync($"{Address}/SetLockoutEndDate", new SetLockoutDTO { User = user, LockoutEnd = EndDate }, cancel);
